	...
	return (
		<div className="bg-white container mx-auto px-10">
			...
			<div className="text-lg font-medium">
				...
				<p>Overall Ratings: 4.6 out of 5</p>
			</div>
			{starsStat && (
				<div className="text-sm mt-3">
					{Object.keys(starsStat).reverse().map((key, i) => {
						return (
							<div key={`rating-${i}`} className="flex items-center gap-3">
								<span>{key} star</span>
								<div class="h-2 relative w-40 rounded mt-0.5 overflow-hidden">
									<div class="w-full h-full bg-gray-200 absolute"></div>
									<div class="h-full bg-yellow-400 absolute" style={{ width: starsStat[key] }}></div>
								</div>
